<div class="col-md-4">
    <div 
        class="box-measurement" 
        data-option-id="<?php echo $post->post_name; ?>">
        
        <div class="measurement-controls diameter-control" data-default="42">
            <div class="row">
                <div class="col-md-4 increase-decrease">
                    <div class="increase">
                        <i 
                            data-table-action="table-increase" 
                            data-table-target="diameter"
                            class="fa fa-chevron-up"></i>
                    </div>
                    <div class="decrease">
                        <i 
                            data-table-action="table-decrease"
                            data-table-target="diameter"
                            class="fa fa-chevron-down"></i>
                    </div>
                </div>
                <div class="col-md-8">
                    <span class="count"><?php the_field('default_size'); ?></span>
                </div>
            </div>
        </div>
        
        <div class="bar">
            <h2 class="title">Diameter (in.)</h2>
        </div>
    </div>
</div>