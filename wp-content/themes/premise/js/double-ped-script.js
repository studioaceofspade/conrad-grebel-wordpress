...
function setTableObject() { 
        ...
        $('.step').each(function(index) {
            ...
            if(canProgress) {      
                ...
                if($step.find('.selected').length > 0 ) {
                    ...
                    if(stepId != 'choose-a-double-pedestal-table' ) {
                        tableData[stepId] = {
    ...
    tableName = tableData['choose-a-double-pedestal-table'].selected[0];
...
function reviewButtonController() {
    $('.review-jump').click(function(e) {
        ...
        $('.step[data-step-id="double-pedestal-review"]').addClass('active');
        $('.render.table').show();
...
function setTablePricing() {
    ...
    var table       = tableData['choose-a-double-pedestal-table'];
    ...
    if(sqin > 3024 && sqin < 3169) {
        dimensionAddon = 150;
    } else if (sqin >= 3169 && sqin < 3456) {
        dimensionAddon = 175;
    } else if (sqin >= 3456 && sqin < 3528) {
        dimensionAddon = 200;
    } else if (sqin >= 3528) {
        dimensionAddon = 225;
    }
...
function configureReview() {
    ...
    woodCost = tableData['choose-a-double-pedestal-table'].pricing.cherry - tableData['choose-a-double-pedestal-table'].pricing.oak;

    oakCost = tableData['choose-a-double-pedestal-table'].pricing.oak * ($('.retailer-meta').data('price-mod') / 100).toFixed(2);
    
    // Model
    $('.model').html(tableData['choose-a-double-pedestal-table'].model);
    
    // Table Style
    $('.dp-table-style').html(tableData['choose-a-double-pedestal-table'].selected[1]);
    if(wood == 'oak' || wood == 'maple') {
        $('.dp-table-style-cost').html('$' + oakCost);
    } else {
        $('.dp-table-style-cost').html('$' + tableData['choose-a-double-pedestal-table'].pricing.cherry * ($('.retailer-meta').data('price-mod') / 100).toFixed(2));
    }
    $('.dp-table-style-edit')
        .html('<a href="#" class="edit" data-edit-step="choose-a-double-pedestal-table">Edit</a>');
    ...
    $('.dp-dimensions-edit')
        .html('<a href="#" class="edit" data-edit-step="set-your-double-pedestal-size">Edit</a>');
        
    // Dimensions with leaves
    $('.dp-dimensions-leaves').html(tableData.dimensions.width + '" wide, ' + tableData.totalLength + '" long with leaves');
...
function loadCompletedTable() {
    ...
    tableData = collection[$('#double-pedestal-table').data('edit')];
    ...
    $('#double-pedestal-table').data('edit','');
}


function setTableSizeRules() {    

    $selected   = $('[data-step-id="choose-a-double-pedestal-table"]').find('.selected').siblings('.option-settings');
    widthArray  = $selected.find('.widths').data('widths').toString().split(',');
    lengthArray = $selected.find('.lengths').data('lengths').toString().split(',');
    
    var tableConstraints = {
        minLeaves   : 0,
        maxLeaves   : $selected.find('.max-leaves').data('leaves'),
        leafSize    : $selected.find('.leaf-size').data('leaf-size'),
        widths      : widthArray,
        lengths     : lengthArray,
    };
    ...
    if(tableSize.length == undefined) {
        ...
        tableSize.leaves    = $('.leaf-control .count').html();
    }
    ...
    if(tableSize.leaves < tableConstraints.minLeaves) {
        tableSize.leaves = tableConstraints.minLeaves;
    }
    if(tableSize.leaves > tableConstraints.maxLeaves) {
        tableSize.leaves = tableConstraints.maxLeaves;
    }
...
function setDoublePedTableSize() {
	...
	$tableRender.width(tableSize.tlength*5.8 + (tableSize.leaves * ts.leafSize * 5.8)); // leaves are 12" wide
    
    $leafPane.empty();
    for(var i = 0; i < tableSize.leaves; i++) {
        $leaf = $('<div class="leaf">');
        $leaf.width(ts.leafSize * 5.8);
        $leafPane.append($leaf);
    }
    
    totalLength = parseInt(tableSize.tlength) + parseInt(tableSize.leaves * ts.leafSize);
...
function setDoublePedChairs() {
	...
	tbChairs = Math.floor((parseInt(tableSize.tlength) + tableSize.leaves * ts.leafSize) / 21) - 1;
