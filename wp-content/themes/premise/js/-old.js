...
function editButtonController() {
    $('.edit').click(function(e) {
        ...
        $('.step[data-step-id="' + $(this).data('edit-step') + '"]').show().addClass('active');
...
function stepPricingControls() {
    ...
    // $('.step[data-display-first-price="1"]').find('.price').hide();
}
...
function setLegTablePricing() {
    ...
    for (var step in legData) {
        ...
        if(step != 'amount' && step != 'skips' && step != 'selectedReference' && step != 'furnitureType') {
    ...
    for (var y = 0; y < percents.length; y++) {
        addonTotal = addonTotal * (1 + percents[y]/100);
    }
    
    legData['amount'] = ((addonTotal + legData.dimensions.price) * ($('.retailer-meta').data('price-mod') / 100)).toFixed(2);
